...
module LemFunction {
  ...
  ******************************************************************************/
  ...
  lemma {:axiom} lem_Exten<A,B>(f1:A->B, f2:A->B)
  ...
  lemma lem_ExtenMul<A,B>(f1:A->real, g1:A->real, f2:A->real, g2:A->real)
    requires forall x:A :: f1(x) == f2(x)
    requires forall x:A :: g1(x) == g2(x)
    ensures (x => f1(x)*g1(x)) == (x => f2(x)*g2(x))
  { 
    lem_Exten(x => f1(x)*g1(x), x => f2(x)*g2(x)) by { 
      forall x:A
  ...
  ******************************************************************************/
  ...
  lemma lem_StrictIncrImpInjective<A>(f:A->A, ord:Ord<A>)
  ...
  lemma lem_StrictDecrImpInjective<A>(f:A->A, ord:Ord<A>)
  ...
  lemma lem_StrictIncrImpInjectiveReal(f:real->real)
    requires forall x, y :: x < y ==> f(x) < f(y)
    ensures  forall x, y :: f(x) == f(y) ==> x == y
  {
  }

  lemma lem_StrictIncrImpInjectiveRealPred(f:real->real, p:real->bool)
    requires forall x, y :: p(x) && p(y) ==> (x < y ==> f(x) < f(y))
    ensures  forall x, y :: p(x) && p(y) ==> (f(x) == f(y) ==> x == y)
  {
  }    

  lemma lem_StrictDecrImpInjectiveReal(f:real->real)
    requires forall x, y :: x < y ==> f(x) > f(y)
    ensures  forall x, y :: f(x) == f(y) ==> x == y
  {
  }

  lemma lem_StrictDecrImpInjectiveRealPred(f:real->real, p:real->bool)
    requires forall x, y :: p(x) && p(y) ==> (x < y ==> f(x) > f(y))
    ensures  forall x, y :: p(x) && p(y) ==> (f(x) == f(y) ==> x == y)
  {
  } 
  ...
  lemma lem_StrictIncrImpInjectiveNat(f:nat->nat)
    requires forall x, y :: x < y ==> f(x) < f(y)
    ensures  forall x, y :: f(x) == f(y) ==> x == y
  {
  }
